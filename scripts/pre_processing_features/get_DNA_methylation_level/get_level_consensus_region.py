...
def args():
  ...
  args_parser.add_argument("-a","--gffFile",help="gff file with promoter/miRNA annotation",type=str,required=True)
  ...
  args_parser.add_argument("-n","--minNumSites",help="minimum number of DNA methylation sites per region; Default: 1 site",type=str,default=1)
...
def getShiftedWindows(chrom,start,end,strand,shift,window_size,optimal_window_file):
  with open(optimal_window_file,'w') as writehandle:
    for i in range(int(start),int((end - window_size + 1)),shift):
      line = chrom + '\t' + str(int(i)) + '\t' + str(int(i+window_size)) + '\t' + '.' + '\t' + '.' + '\t' + strand + '\n'
...
def calculateOptimalWindow(chrom,middle,strand,bedFile,mapping_betaValues,region):
  ...
  shifted_windows = getShiftedWindows(chrom,scanningRegion_start,scanningRegion_end,strand,shift,window_size,optimal_window_file)
  
  bedCmd = "intersectBed -wo -s -a " + optimal_window_file + " -b " + bedFile + " > " + bed_analysis_results
  os.system(bedCmd)
...
def getOptimalWindow(chrom,start,end,strand,bedFile,mapping_betaValues):
  ...
  upstream_interval = calculateOptimalWindow(chrom,middle,strand,bedFile,mapping_betaValues,'u')
  downstream_interval = calculateOptimalWindow(chrom,middle,strand,bedFile,mapping_betaValues,'d')
...
def getConsensusRegion(gffFile,bedFile,mapping_betaValues):
  ...
  with open(bed_region,'w') as writehandle:
    for line in readhandle:
      ...
      promoter_newRegion = getOptimalWindow(line[0],line[3],line[4],line[6],bedFile,mapping_betaValues)
  ...
  bedCmd = "intersectBed -wo -a " + bed_region + " -b " + bedFile + " > " + bedAnalysisFile
  os.system(bedCmd)
  ...
  with open(DNAmethFile,'w') as writehandle:
    for line in readhandle:
      ...
      if promID in IDtoDNAmeth:
        DNAmeth_coverage = float(round(numpy.mean(IDtoDNAmeth[promID]),3))
        CpG_prom = 'CpG'
